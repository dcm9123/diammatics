...
negative_contamination = function(ps_object){
  ps_neg_genus = tax_glom(ps_object, taxrank="genus_final", NArm = FALSE)
  ps_neg_genus_df = as.data.frame(tax_table(ps_neg_genus))
  cat("--------GENUS--------","\n")
  cat("The genera found in the negative control are ",nrow(ps_neg_genus_df),": ","\n")
  cat((ps_neg_genus_df$genus_final),sep = ", \n","\n")
  ps_neg_species = tax_glom(ps_object, taxrank = "species_final", NArm = FALSE)
  ps_neg_species_df = as.data.frame(tax_table(ps_neg_species))
  ps_neg_species_df$genus_and_species = paste(ps_neg_species_df$genus_final,ps_neg_species_df$species_final, sep=" ")
  cat("--------SPECIES--------")
  cat("The species found in the negative control are ",nrow(ps_neg_species_df),": ","\n")
  cat((ps_neg_species_df$genus_and_species),sep = ", \n","\n")
}
...
# sample_data(ps_merged_glom)
# pos2 = subset_samples(ps_merged_glom, sample_data(ps_merged_glom)$ID=="Plate2_Positive_control_S89_L001")
# pos2 = prune_taxa(taxa_sums(pos2)>0, pos2)
# tax_table(pos2)
# 
# test = subset_samples(ps2, sample_data(ps2)$ID=="Plate2_Positive_control_S89_L001")
...
#individual ps (i.e. ps_pos1, ps_pos2, etc.), but the argument wasn't recognized inside the subset_samples phyloseq function ARGHHHH!
...
ps_pos2 = subset_samples(ps_merged_positive, sample_data(ps_merged_positive)$ID=="Plate2_Positive_control_S89_L001")
ps_pos2 = prune_taxa(taxa_sums(ps_pos2)>0, ps_pos2)
ps_pos3 = subset_samples(ps_merged_positive, sample_data(ps_merged_positive)$ID=="Plate3_Positive_Control_S89_L001")
ps_pos3 = prune_taxa(taxa_sums(ps_pos3)>0, ps_pos3)
ps_pos4 = subset_samples(ps_merged_positive, sample_data(ps_merged_positive)$ID=="Plate4_Positive_Control_S89_L001")
ps_pos4 = prune_taxa(taxa_sums(ps_pos4)>0, ps_pos4)
ps_pos5 = subset_samples(ps_merged_positive, sample_data(ps_merged_positive)$ID=="Plate5_Positive_Control_S89_L001")
ps_pos5 = prune_taxa(taxa_sums(ps_pos5)>0, ps_pos5)

ps_neg1 = subset_samples(ps_merged_negative, sample_data(ps_merged_negative)$ID=="Plate1_negative_control_S90_L001")
ps_neg1 = prune_taxa(taxa_sums(ps_neg1)>0, ps_neg1)
ps_neg2 = subset_samples(ps_merged_negative, sample_data(ps_merged_negative)$ID=="Plate2_negative_control_S90_L001")
ps_neg2 = prune_taxa(taxa_sums(ps_neg2)>0, ps_neg2)
ps_neg3 = subset_samples(ps_merged_negative, sample_data(ps_merged_negative)$ID=="Plate3_Negtive_Control_S90_L001")
ps_neg3 = prune_taxa(taxa_sums(ps_neg3)>0, ps_neg3)
ps_neg4 = subset_samples(ps_merged_negative, sample_data(ps_merged_negative)$ID=="Plate4_Negtive_Control_S90_L001")
ps_neg4 = prune_taxa(taxa_sums(ps_neg4)>0, ps_neg4)
ps_neg5 = subset_samples(ps_merged_negative, sample_data(ps_merged_negative)$ID=="Plate5_negative_ctrl_S90_L001")
ps_neg5 = prune_taxa(taxa_sums(ps_neg5)>0, ps_neg5)
ps_neg3_ex = subset_samples(ps_merged_negative, sample_data(ps_merged_negative)$ID=="Plate3_Negative_extraction_S85_L001")
ps_neg3_ex = prune_taxa(taxa_sums(ps_neg3_ex)>0, ps_neg3_ex)
ps_neg4_ex = subset_samples(ps_merged_negative, sample_data(ps_merged_negative)$ID=="Plate4_Negative_extraction_S85_L001")
ps_neg4_ex = prune_taxa(taxa_sums(ps_neg4_ex)>0, ps_neg4_ex)

engraftment_and_contamination_positives(ps_pos1) 
engraftment_and_contamination_positives(ps_pos2) 
engraftment_and_contamination_positives(ps_pos3) 
engraftment_and_contamination_positives(ps_pos4) 
engraftment_and_contamination_positives(ps_pos5) 

negative_contamination(ps_neg1)
negative_contamination(ps_neg2)
negative_contamination(ps_neg3)
negative_contamination(ps_neg3_ex)
negative_contamination(ps_neg4)
negative_contamination(ps_neg4_ex)
negative_contamination(ps_neg5)
