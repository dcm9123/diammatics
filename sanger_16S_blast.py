...
os.chdir(path)
master_file = pd.read_csv(path+"FINAL_report_WGS_all_csv_April.csv")
subset_master_file = master_file[['Sample ID', 'Sanger classification (v3v4) Source','Selected for Downstream','Classification species']]
...
my_dict={}
for file in os.listdir(path+"fixed_assembly_benchling"):                                                                #This section filters by identity and e-value, and gets unique species names. The ID of the file and species is appended to a dictionary.
    if file.endswith(".csv") and file.startswith("S"):
        df = pd.read_csv(path+"fixed_assembly_benchling/"+file)                                                         #Each df is the BLAST output file for each sample
...
for sample_id,(species,genus,sequence) in my_dict.items():
    ...
    gtdbtk_genus = subset_master_file.loc[sample_id,'Classification species'].split(" ")[0]
